...
public interface IMqConnection extends IObject
{
   ...
   * First we verify the client is actually connected, otherwise there's no point
   * in disconnecting.<br>
   * Note we allocate a new {@link Socket} following the call to {@link Socket#close()}
   * because a closed socket cannot be reused.
   */
   ...
   * 
   * @return
   *   the last message the {@link IClient} issued for a call.
   */
